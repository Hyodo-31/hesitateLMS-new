        ...
        <main class="page-content teacher-form-page">
            ...
            <h2>学生グループ作成</h2>
                ...
                <form id="search-form" method="GET" hidden aria-hidden="true">
                    ...
                    <section id="histogram-search-method-panel" class="filter-conditions-accordion histogram-filter-accordion" aria-label="ヒストグラムでの絞り込み" hidden>
                    ...
                    <section class="histogram-dashboard" aria-labelledby="histogram-dashboard-title">
                        <div class="histogram-dashboard-heading">
                            <div>
                                ...
                                <p>WID、UID、選択結果の順に条件を作成します。正答率・迷い率は各ヒストグラムの指標欄から選択できます。</p>
                            </div>
                            <details class="histogram-axis-details">
                                <summary>軸の自動調整について</summary>
                                <p>横軸は表示対象の最小値・最大値、四分位範囲、件数から階級幅を毎回計算します。</p>
                            </details>
                        </div>

                        <details class="histogram-workflow-step" open>
                            <summary class="histogram-workflow-summary"><span class="histogram-workflow-number">1</span><span><strong>WIDに関する検索</strong><small>問題の選択からWID縦棒の論理式までを設定します。</small></span><span class="histogram-workflow-icon" aria-hidden="true"></span></summary>
                            <div class="histogram-workflow-body">
                                <section class="histogram-workflow-stage">
                                    <div class="histogram-stage-heading"><span>①</span><div><h4>WIDの選択</h4><p>UID検索に使用する問題をチェックしてください。</p></div></div>
                                    <div class="button-container histogram-wid-buttons"><button type="button" id="histogram-select-all-wid-btn">すべて選択</button><button type="button" id="histogram-deselect-all-wid-btn">すべて解除</button></div>
                                    ...
                                    <?php foreach ($student_filter_wids as $row): ?>
                                        ...
                                        ?>
                                        <div class="list-item histogram-wid-list-item"><label class="student-choice"><input type="checkbox" class="histogram-wid-checkbox" value="<?= $histogram_wid ?>" checked><span class="student-name"><?= $histogram_wid_label ?></span></label></div>
                                    <?php endforeach; ?>
                                    </div>
                                </section>

                                <section class="histogram-workflow-stage">
                                    <div class="histogram-stage-heading"><span>②</span><div><h4 id="wid-feature-histogram-title">WIDのヒストグラム</h4><p>特徴量・正答率・迷い率のWID分布を表示します。</p></div></div>
                                    <article class="histogram-card" aria-labelledby="wid-feature-histogram-title">
                                        <div class="histogram-controls"><label>特徴量・指標<select id="wid-feature-histogram-feature"></select></label><label>対象UID<select id="wid-feature-histogram-uid-scope"><option value="all" selected>全UID</option><option value="checked">選択UID</option></select></label><label>対象WID<select id="wid-feature-histogram-wid-scope"><option value="checked" selected>チェック中のWID</option><option value="all">全WID</option></select></label></div>
                                        <div class="histogram-chart-wrap"><canvas id="wid-feature-histogram-chart" aria-label="WIDごとの特徴量または指標の分布"></canvas></div>
                                        <p id="wid-feature-histogram-summary" class="histogram-summary" aria-live="polite"></p>
                                    </article>
                                </section>

                                <section class="histogram-workflow-stage">
                                    <div class="histogram-stage-heading"><span>③</span><div><h4>WID縦棒の論理式</h4><p>論理式の結果は、ボタンを押したときだけ①のWIDチェックへ反映されます。</p></div></div>
                                    <div id="histogram-wid-bar-logic-panel" class="logic-filter-panel histogram-bar-logic-panel">
                                        <div class="logic-filter-toolbar"><button type="button" data-add-histogram-bar-logic="wid-condition">縦棒を追加</button><button type="button" data-add-histogram-bar-logic="wid-and">AND</button><button type="button" data-add-histogram-bar-logic="wid-or">OR</button><button type="button" data-add-histogram-bar-logic="wid-not">NOT</button><button type="button" data-add-histogram-bar-logic="wid-open">(</button><button type="button" data-add-histogram-bar-logic="wid-close">)</button><label>追加位置<select id="histogram-wid-bar-logic-insert-position" class="logic-filter-insert-position"></select></label></div>
                                        <div id="histogram-wid-bar-logic-builder" class="logic-filter-builder"></div>
                                        <div class="logic-filter-actions"><button type="button" id="reset-histogram-wid-bar-logic">OR式に戻す</button><button type="button" id="clear-histogram-wid-bar-logic" class="logic-filter-clear">式を空にする</button><button type="button" id="apply-histogram-wid-bar-result" disabled>論理式の結果をWIDチェックへ反映</button><span id="histogram-wid-bar-logic-summary" class="logic-filter-summary">WIDの縦棒は選択されていません。</span></div>
                                        <div class="histogram-saved-wid-bars"><div class="histogram-saved-wid-bars-heading"><strong>保存したWID縦棒</strong><button type="button" id="clear-saved-histogram-wid-bars" class="logic-filter-clear">保存情報をすべて削除</button></div><div id="histogram-saved-wid-bars-list" class="histogram-saved-wid-bars-list" aria-live="polite"><span class="histogram-saved-wid-bars-empty">保存したWID縦棒はありません。</span></div></div>
                                    </div>
                                </section>

                                <div class="histogram-wid-apply-actions"><div><strong>WIDの選択を確定</strong><span id="histogram-wid-filter-apply-summary" class="histogram-wid-filter-apply-summary" aria-live="polite"><?= count($student_filter_wids) ?>件のWIDをUID検索へ反映しています。</span></div><button type="button" id="apply-histogram-wid-filter">選択したWIDをUID検索へ反映</button></div>
                            </div>
                        </details>

                        <details class="histogram-workflow-step" id="histogram-uid-workflow-step">
                            <summary class="histogram-workflow-summary"><span class="histogram-workflow-number">2</span><span><strong>UIDに関する検索</strong><small>学習者の選択からUID縦棒の論理式までを設定します。</small></span><span class="histogram-workflow-icon" aria-hidden="true"></span></summary>
                            <div class="histogram-workflow-body">
                                <section class="histogram-workflow-stage">
                                    <div class="histogram-stage-heading"><span>④</span><div><h4>論理式とUIDの選択</h4><p>グループ条件の論理式、またはチェックボックスで対象UIDを選択します。</p></div></div>
                                    <div id="histogram-uid-logic-filter-panel" class="logic-filter-panel"><strong>論理式で対象UIDを選択</strong><div class="logic-filter-toolbar"><button type="button" data-add-histogram-uid-filter="condition">対象を追加</button><button type="button" data-add-histogram-uid-filter="and">AND</button><button type="button" data-add-histogram-uid-filter="or">OR</button><button type="button" data-add-histogram-uid-filter="not">NOT</button><button type="button" data-add-histogram-uid-filter="open">(</button><button type="button" data-add-histogram-uid-filter="close">)</button><label>追加位置<select id="histogram-uid-logic-filter-insert-position" class="logic-filter-insert-position"></select></label></div><div id="histogram-uid-logic-filter-builder" class="logic-filter-builder"></div><div class="logic-filter-actions"><button type="button" id="reset-histogram-uid-logic-filter">リセット</button><button type="button" id="trim-histogram-uid-logic-filter" class="logic-filter-trim">追加位置から後ろを削除</button><button type="button" id="clear-histogram-uid-logic-filter" class="logic-filter-clear">式を空にする</button><span id="histogram-uid-logic-filter-summary" class="logic-filter-summary">すべての学習者を対象にしています。</span></div></div>
                                    <div id="histogram-uid-checkbox-list" class="checkbox-section histogram-checkbox-section"><div class="checkbox-controls"><label><input type="checkbox" class="histogram-select-all" checked> 全ての表示中学習者を 選択 / 解除</label></div><div class="checkbox-list histogram-uid-checkbox-grid">
                                    <?php
                                    ...
                                    foreach ($student_filter_rows as $row) {
                                        if ($histogram_current_class_id !== (string)$row['ClassID']) {
                                            ...
                                            $safe_class_id = htmlspecialchars($row['ClassID'], ENT_QUOTES, 'UTF-8');
                                            $safe_class_name = htmlspecialchars($row['ClassName'], ENT_QUOTES, 'UTF-8');
                                            echo "<div class='class-group-header' data-class-id='{$safe_class_id}'><h5>{$safe_class_name}</h5><label><input type='checkbox' class='histogram-select-all-class' data-class-id='{$safe_class_id}' checked> このグループ(クラス)を全て選択 / 解除</label></div>";
                                        }
                                        $safe_class_id = htmlspecialchars($row['ClassID'], ENT_QUOTES, 'UTF-8');
                                        $uid = htmlspecialchars($row['uid'], ENT_QUOTES, 'UTF-8');
                                        $name = htmlspecialchars($row['Name'], ENT_QUOTES, 'UTF-8');
                                        echo "<label class='checkbox-item histogram-uid-filter-item student-choice' data-class-id='{$safe_class_id}' data-student-name='{$name}'><input type='checkbox' class='histogram-uid-checkbox' value='{$uid}' checked><span class='student-name'><span class='label-text'>UID:</span> {$uid} / <span class='label-text'>名前:</span> {$name}</span></label>";
                                    }
                                    ?>
                                    </div></div>
                                </section>
                                ...
                                <section class="histogram-workflow-stage">
                                    <div class="histogram-stage-heading"><span>⑤</span><div><h4 id="uid-feature-histogram-title">UIDのヒストグラム</h4><p>特徴量・正答率・迷い率のUID分布を表示します。</p></div></div>
                                    <article class="histogram-card" aria-labelledby="uid-feature-histogram-title"><div class="histogram-controls"><label>特徴量・指標<select id="uid-feature-histogram-feature"></select></label><label>対象UID<select id="uid-feature-histogram-uid-scope"><option value="checked" selected>チェック中のUID</option><option value="all">全UID</option></select></label><label>対象WID<select id="uid-feature-histogram-wid-scope"><option value="checked" selected>反映済みのWID</option><option value="all">全WID</option></select></label></div><div class="histogram-chart-wrap"><canvas id="uid-feature-histogram-chart" aria-label="UIDごとの特徴量または指標の分布"></canvas></div><p id="uid-feature-histogram-summary" class="histogram-summary" aria-live="polite"></p></article>
                                </section>

                                <section class="histogram-workflow-stage">
                                    <div class="histogram-stage-heading"><span>⑥</span><div><h4>UID縦棒の論理式</h4><p>論理式の結果は、下の選択結果へ自動で反映されます。</p></div></div>
                                    <div id="histogram-uid-bar-logic-panel" class="logic-filter-panel histogram-bar-logic-panel"><div class="logic-filter-toolbar"><button type="button" data-add-histogram-bar-logic="uid-condition">縦棒を追加</button><button type="button" data-add-histogram-bar-logic="uid-and">AND</button><button type="button" data-add-histogram-bar-logic="uid-or">OR</button><button type="button" data-add-histogram-bar-logic="uid-not">NOT</button><button type="button" data-add-histogram-bar-logic="uid-open">(</button><button type="button" data-add-histogram-bar-logic="uid-close">)</button><label>追加位置<select id="histogram-uid-bar-logic-insert-position" class="logic-filter-insert-position"></select></label></div><div id="histogram-uid-bar-logic-builder" class="logic-filter-builder"></div><div class="logic-filter-actions"><button type="button" id="reset-histogram-uid-bar-logic">OR式に戻す</button><button type="button" id="clear-histogram-uid-bar-logic" class="logic-filter-clear">式を空にする</button><span id="histogram-uid-bar-logic-summary" class="logic-filter-summary">UIDの縦棒は選択されていません。</span></div><div class="histogram-saved-uid-bars"><div class="histogram-saved-uid-bars-heading"><strong>保存したUID縦棒</strong><button type="button" id="clear-saved-histogram-uid-bars" class="logic-filter-clear">保存情報をすべて削除</button></div><div id="histogram-saved-uid-bars-list" class="histogram-saved-uid-bars-list" aria-live="polite"><span class="histogram-saved-uid-bars-empty">保存したUID縦棒はありません。</span></div></div></div>
                                </section>
                            </div>
                        </details>

                        <section class="histogram-workflow-result"><div class="histogram-stage-heading"><span class="histogram-workflow-number">3</span><div><h4>ヒストグラム選択結果</h4><p>⑥のUID縦棒論理式の結果が、下の学習者リストへ自動で反映されます。</p></div></div></section>
                    </section>
                        </div>
                    </section>
