...
if (preg_match("/。$/u", $Japanese) || preg_match("/\?$/u", $Japanese) || preg_match("/？$/u", $Japanese)) { // 日本文の末尾が.か?で終わっていなかったら
} else {
    $flag[0] = 1;
}

if (preg_match("/\.$/", $Sentence) || preg_match("/\?$/", $Sentence) || preg_match("/!$/", $Sentence)) { // 英文の終わりが.か?か!で終わっていなかったら
} else {
    $flag[1] = 1;
}
...
if($error == 0){
	...
	echo "<b>" . translate('entry.php_60行目_問題文') . "</b>：" . htmlspecialchars($Japanese, ENT_QUOTES, 'UTF-8') . "<br>";
	echo "<b>" . translate('entry.php_61行目_英文') . "</b>：" . htmlspecialchars($Sentence, ENT_QUOTES, 'UTF-8') . "<br>";
